...
| Deploy button secrets | collected from **`.dev.vars.example` / `.env.example`** in dotenv format; the help text beside each field is `cloudflare.bindings.<NAME>.description` in `package.json`, and supports inline markdown `code`, `**bold**`, `__italics__` and `[links]` | 2026-07-25 |
| Deploy button and optional secrets | **there is none.** Every field the form shows blocks deploy if left blank, and no mechanism exists to mark one optional — not a `required` flag, not an `# OPTIONAL` marker, not an empty default. Not stated in the docs either way; the source is [workers-sdk#14075](https://github.com/cloudflare/workers-sdk/issues/14075), open, reproduced against the live form on 2026-05-28, whose author's workaround was to delete `.dev.vars.example`. The consequence is the design rule: **a secret goes in that file only if a value is required *and* any value the deployer invents is safe.** Otherwise it leaves the file and becomes a README `wrangler secret put` step, which `pnpm check:deploy` enforces (#139) | 2026-07-25 |
| Deploy button and `vars` | environment variables **are** collected too, and unmasked — "These secrets and environment variables will be presented to users in the dashboard as they deploy this template, allowing them to configure each value" ([changelog, 2025-07-01](https://developers.cloudflare.com/changelog/post/2025-07-01-workers-deploy-button-supports-environment-variables-and-secrets/)). Established for #139 and **deliberately not used**: it would split the email-notification trio across `wrangler.jsonc` `vars` and a secret, and `wrangler types` would then generate the two names into `src/worker-configuration.d.ts` where `src/notify/env.ts` already declares them | 2026-07-25 |
| `wrangler types` and `.dev.vars` | folds a local `.dev.vars`'s keys into the generated `Env`, and `--check` regenerates to compare — so a local secrets file both false-fails `types:check` and gets written into the committed contract by the obvious fix. **`--env-file scripts/no-secrets.env` on both scripts** is the fix (#132); the `--check` codepath honouring it is newer than [workers-sdk#13679](https://github.com/cloudflare/workers-sdk/issues/13679), verified working on wrangler 4.114.0. A **nonexistent** path is a hard failure, so the file is committed rather than being `/dev/null`, which does not exist on Windows. `wrangler.jsonc`'s `secrets.required` also stops the inference, and is **not** used here: it makes `wrangler deploy` fail when a listed secret is unset, which on a first one-click deploy is unverifiable and would fail closed on strangers | 2026-07-25 |
...
| Workers Builds API token | the token created by **Create new token** in Workers Builds settings carries Account Settings (read), Workers Scripts (edit), Workers KV (edit), Workers R2 (edit) — **D1 is not on that documented list**. What the Deploy-button flow's own token carries is *not* documented. Matters because `wrangler d1 migrations apply` exits non-zero with no useful output when its token lacks D1 (workers-sdk#5077), so a first deploy must read the build log for that step | 2026-07-25 |
...
| Cache API availability | documented: **not available** for Workers fronted by Cloudflare Access. Inferred, not stated: unusable on `workers.dev`, because the availability sentence enumerates custom domains and `*.pages.dev` and omits `*.workers.dev`. Either way it is not a primitive to build a one-click-deploy guard on ([#124](https://github.com/withsetu/charcha/issues/124)) | 2026-07-25 |
...
`pnpm check` is the whole gate — types, typecheck, lint, format, tests, embed
budget — and is exactly what CI runs. Run it before you push.

**The package manager is pnpm, and that is a fix rather than a taste.** npm's
`package-lock.json` records the tree installed on the *resolving* machine: on
macOS/arm64 npm resolves sharp's native binary and omits the wasm fallback's
`@emnapi/*` deps, the Linux runner needs them, and `npm ci` fails closed before a
single test runs. It broke CI twice before [#52](https://github.com/withsetu/charcha/issues/52)
...
| Thread key | `src/page-key.ts` — the only way a URL becomes a `page_key`. Derived in the Worker, never accepted from the embed |
| Renderer | `src/render/` — rows to an HTML string, pure; the class names it emits are a public API |
| Public read path | `src/read/route.ts` — `GET /comments`, HTML out; the origin allowlist and preflight are `src/cors.ts` |
| Origin policy | `src/cors.ts` — the allowlist, and the same-origin default that makes a fresh deployment accept a comment without a settings row (#57). `selfOrigin` is the one definition of "this deployment", shared with `src/admin/csrf.ts` |
| Root page | `src/root/page.ts` — `GET /`, the page the deploy-success link lands on (#140). A **constant**: it says Charcha is running, what Charcha is, and links to charcha.dev — and nothing about the deployment it is running on. It reads no binding and interpolates nothing from the request, because the address is public and an operational readout there is a leak (#145). Anything a deployer needs goes in the README |
| Schema | `migrations/` — applied with `pnpm db:migrate`; tests run against these same files |
| Wrangler config | `wrangler.jsonc` — D1 binding `DB`, database `charcha` |
| Binding types | `src/worker-configuration.d.ts`, generated by `pnpm types`; CI fails if it drifts. Both type scripts read `scripts/no-secrets.env` and nothing else, so a local `.dev.vars` can neither fail the check nor leak a name into the committed contract (#132) — `test/node/wrangler-types.test.ts` asserts the pairing and that the file stays empty |
...
| One-click deploy | `.dev.vars.example` is the secret form and `cloudflare.bindings` in `package.json` is its help text — but only for the secrets that belong on a form, which since #139 is `CHARCHA_DASHBOARD_PASSWORD` and `IP_HASH_SECRET` and nothing else. The rest are commented out there and set from README's **Turning on the optional features**, so **README.md is now part of this contract**: `pnpm check:deploy` fails a secret that is in neither place, and fails a description left behind by a field that no longer exists. Which side each secret is on is `DEPLOY_FORM_SECRETS`, asserted both ways, so moving one is an edit to that list rather than to a dotenv file. `deploy` migrates before it deploys. All of it in `scripts/deploy-config.mjs` |
