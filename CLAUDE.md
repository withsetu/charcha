...
| `wrangler types` and `.dev.vars` | folds a local `.dev.vars`'s keys into the generated `Env`, and `--check` regenerates to compare — so a local secrets file both false-fails `types:check` and gets written into the committed contract by the obvious fix. **`--env-file` on both scripts** is the fix (#132); the `--check` codepath honouring it is newer than [workers-sdk#13679](https://github.com/cloudflare/workers-sdk/issues/13679), verified working on wrangler 4.114.0. `wrangler.jsonc`'s `secrets.required` also stops the inference, and is **not** used here: it makes `wrangler deploy` fail when a listed secret is unset, which on a first one-click deploy is unverifiable and would fail closed on strangers | 2026-07-25 |
...
| Binding types | `src/worker-configuration.d.ts`, generated by `pnpm types`; CI fails if it drifts |
